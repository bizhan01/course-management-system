      ...
      <table class="table table-striped table-bordered dataTable" id="table-2">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <center>
              @if($fee->created_at == \Carbon\Carbon::now())
                <span class="btn btn-rounded btn-sm btn-success">
                  111
                </span>
              @elseif($fee->created_at >= \Carbon\Carbon::now()->subdays(15))
                <button class="btn btn-rounded btn-sm btn-warning">
                  {{ \Carbon\Carbon::parse($fee->created_at)->diffForHumans() }}
                </button>
              @else
                <span class="btn btn-rounded btn-sm btn-danger">
                  {{ \Carbon\Carbon::parse($fee->created_at)->diffForHumans() }}
                </button>
              @endif
            </center>
